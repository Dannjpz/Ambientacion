...
create procedure "informix".sp_registra_evento (pTipoMsj char(1),
pIdMsj char(10), 
pNumclt char(20),
pNumcta char(20),
pNumTarjeta char(16),
...
PFECHA2 datetime year to fraction(3)
    ...
    -- Realizo   :Angel Rene de la Llave
    -- Proyecto : Latinia registro de eventos.
    -- Actividad : Se registran los eventos para el envÃƒÂ­o de mnsjr y emails a un cliente
    --                  basados en las transacciones que haya efectuado.
    -- Fecha     : 26/03/2012
    -- ModificaciÃƒÂ³n: Incluir campos string adicionales. JGP-19/09/2012
    -- Fecha:       19/09/2012
    -- ModificaciÃƒÂ³n: Incluir registro de eventos pendientes de confirmar "PENDIENTE", hasta que sean 
    --               confirmados por Intercard. JGP-09/11/2012
    -- Fecha:       09/11/2012
    --*******************************************************************************************************
 
--DefiniciÃƒÂ³n de Variables
DEFINE cCodRet CHAR(5);
DEFINE iexiste INTEGER;
DEFINE iexiste2 INTEGER;
DEFINE iexiste3 INTEGER;
...
--VALIDA QUE SI NO CONTIENE COMO PARAMETRO DE ENTRADA EL NUMERO DE CLIENTE, BÃƒ?SQUE POR TARJETA O CUENTA.
	...
	-- Eventos que requieren confirmaciÃƒÂ³n se registran como temporales. JGP-17/09/2012
	/*IF pIdMsj IN ('POS_DEBS','POS_CREDS','ATM_DEBS','ATM_CREDS','POS_DEBE','POS_CREDE','ATM_DEBE','ATM_CREDE') THEN -- Posible MigraciÃƒÂ³n a Tabla
		LET vtransaction_id ='PENDIENTE';
	ELSE*/
		LET vtransaction_id = NULL;
	--END IF;	
 
 --ValidaciÃƒÂ³n para saber a que canal corresponde la notificaciÃƒÂ³n(BPI ÃƒÂ³ EMPRESANET) --AÃƒÂ­da Valenzuela
...
RETURNING CHAR(5) as cCodRet;  -- Codigo de Retorno.
    ...
    -- Realizo   :Angel Rene de la Llave
    -- Proyecto : Latinia registro de eventos.
    -- Actividad : Se registran los eventos para el envÃƒÂƒ?ÃƒÂ‚?ÃƒÂƒ?ÃƒÂ‚?ÃƒÂƒ?ÃƒÂ‚?ÃƒÂƒ?ÃƒÂ‚Ã‚Â­o de mnsjr y emails a un cliente
    --                  basados en las transacciones que haya efectuado.
    -- Fecha     : 26/03/2012
    -- Modificacion: Incluir campos string adicionales. JGP-19/09/2012
    -- Fecha:       19/09/2012
    -- ModificaciÃƒÂƒ?ÃƒÂ‚?ÃƒÂƒ?ÃƒÂ‚?ÃƒÂƒ?ÃƒÂ‚?ÃƒÂƒ?ÃƒÂ‚Ã‚Â³n: Incluir registro de eventos pendientes de confirmar "PENDIENTE", hasta que sean 
    --               confirmados por Intercard. JGP-09/11/2012
    -- Fecha:       09/11/2012
	-- Realizo  : Manuel Osuna V.  
	-- ModificaciÃƒÂƒ?ÃƒÂ‚?ÃƒÂƒ?ÃƒÂ‚Ã‚Â³n: Se agrega parametro pIdPlantilla,para que explotar la funcionalidad de que una alerta
	-- pueda manejar multiples plantillas.
    -- Fecha:       15/10/2013
	-- Realizo  : Cristo Lugo  
	-- ModificaciÃƒÂƒ?ÃƒÂ‚?ÃƒÂƒ?ÃƒÂ‚Ã‚Â³n: Se agrega validaciÃƒÂƒ?ÃƒÂ‚?ÃƒÂƒ?ÃƒÂ‚Ã‚Â³n pIdMsj,para que evitar enviar la misma alerta durante el dia.
    -- Fecha:       28/08/2014
    --*******************************************************************************************************
 
--DefiniciÃƒÂƒ?ÃƒÂ‚?ÃƒÂƒ?ÃƒÂ‚?ÃƒÂƒ?ÃƒÂ‚?ÃƒÂƒ?ÃƒÂ‚Ã‚Â³n de Variables
DEFINE cCodRet CHAR(5);
DEFINE iexiste INTEGER;
DEFINE iexiste2 INTEGER;
DEFINE iexiste3 INTEGER;
...
BEGIN
   ON EXCEPTION SET vsqlerr
      ...
      END IF;
   END EXCEPTION;
--	SET DEBUG FILE TO "/tmp/cristo/sp_registra_evento3.out";
--	TRACE ON;
...
--- ENVIO FORZADO DE PUSH Y SMS A CLIENTES ESPECIFICOS PARA NOTIFIACIONES DE COMPRAS CON TARJETA DEBITO Y CREDITO --- Se desactiva funcionalidad por activacion de INOTS 17-09-2024
...
--VALIDA QUE SI NO CONTIENE COMO PARAMETRO DE ENTRADA EL NUMERO DE CLIENTE, BÃƒÂƒ?ÃƒÂ‚?ÃƒÂƒ?ÃƒÂ‚?ÃƒÂƒ?ÃƒÂ‚?ÃƒÂƒ?ÃƒÂ‚Ã‚Â¡SQUE POR TARJETA O CUENTA.
  ...
  ---SE ASIGNA PRIORIDAD DE LECTURA A LA ALERTA
	...
	--SYNDEIN
		...
		IF(iprioridad IS NULL or iprioridad = '') THEN
				LET iprioridad = 9;
		END IF;
		LET vInsStmt = "INSERT INTO " || vTablaNotif || "(tipo_mensaje, id_mensaje,id_plantilla,cliente, cuenta, tarjeta, transaction_id," ||
		"estatus, fecha_hora_registro, fecha_hora_recuperado, " ||
		"string1, string2, string3, string4, string5, string6, string7, string8, string9, string10," || 
		"correo_alterno, celular_alterno, importe1, importe2, importe3, importe4, importe5, fecha1, fecha2, prioridad) "  ||
