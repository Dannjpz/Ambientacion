DROP PROCEDURE IF EXISTS "informix".sp_limite_max( CHAR(10),
                                           CHAR(16),
                                           CHAR(2),
                                           CHAR(2),         -- 01 - ATM, 02 - POS, 03 - PORTAL, 15 - EMPRESANET, 17 - BANCOPPEL MOVIL ONLINE.
                                           DATE,
                                           DECIMAL(16,2),
                                           CHAR (20),
										        char(16),
...
-- Declaracion de variables
	...
	--SET DEBUG FILE TO "/resplogifx/repaclaraciones/sp_limite_maxv2.out";
	--TRACE ON;
...
BEGIN
    ...
    IF (pNumcte is null OR pNumcte = '') OR --LENGTH(pNumcte) <> 10) OR
       ...
       (pMto_tot is null OR pMto_tot <= 0.00) THEN
        ...
        LET vcodret1 = '00030';
        LET vMensaje1 = 'Se genero algÃºn error en la ejecucion';
	...
	IF pCanal = '03' OR pCanal = '15' OR pCanal = '17' THEN		-- BGM SE AGREGA VALIDACIÃN DE CANAL PARA ASIGNAR EL VALOR INDICADO EN VOPERACION
	...
	-- Si ya existe un registro en si_limite_diario, se asegura que el campo importe_dia tenga un valor vÃ¡lido.
	...
	IF vlimite_personalizado_rest2=1 and pMto_tot > vMax_pesos2 THEN --si existe un limite por operacion,vMax_pesos es de si_plimites_empresas
		...
		LET vMensaje1 = 'Importe de la operacion excede el lÃ­mite diario permitido para el transaccion';
		LET vIndicador = '1';
		RETURN vCodret1, vMensaje1, vIndicador;
	...
	ELIF vlimite_personalizado_rest1=1 and (pMto_tot + vImporte) > vMax_pesos1 THEN --Si existe limite por cuenta-operacion, 
		...
		LET vMensaje1 = 'Importe de la operacion excede el lÃ­mite diario permitido para el cuenta';
		LET vIndicador = '1';
		RETURN vCodret1, vMensaje1, vIndicador;

	ELIF vlimite_personalizado_rest1=0 and (pMto_tot + vImporte) > vMax_pesos   THEN --se agrega el if vlimite_personalizado=0
		LET vcodret1 = '00035';
		LET vMensaje1 = 'Importe de la transaccion excede el lÃ­mite diario permitido para el canal';
		LET vIndicador = '1';
