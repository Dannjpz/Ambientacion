DROP PROCEDURE IF EXISTS "informix".abono_ref(      CHAR(3),
                                           CHAR(4),
                                            CHAR(8),
                                           CHAR(4),
                                            CHAR(4),
...
CREATE PROCEDURE "informix".abono_ref( pempresa     CHAR(3),
                                       psucursal    CHAR(4),
                                       pusuario     CHAR(8),
                                       ptransacc    CHAR(4),
                                       ptransuc     CHAR(4),
...
RETURNING CHAR(5);
    ...
    --SET DEBUG FILE TO "/resplogifx/repaclaraciones/abono_ref.out";
	--TRACE ON;
    ...
    --- SET DEBUG FILE TO "/resplogifx/conciliachq/abono_ref.out";
    --- TRACE ON;
    ...
    IF (ptransuc = '0253') THEN 
        IF pmto_firme > 0 THEN
            LET pmto_sbc = pmto_firme;
            LET pmto_firme = 0;
        END IF;
    END IF;
        ...
        FOREACH abono_cursor FOR
            ...
            IF ( ptransacc IN('0273','0446') AND vproducto NOT IN('1200','1600','1800','2200','2600','2700') ) THEN
                SELECT valor
                  INTO vValRetSpei
                  FROM sc_param
                 WHERE codparam = 'ValidaMovsRetDepSpei';
                 
                IF ( vValRetSpei is null OR vValRetSpei = '' ) THEN
                    LET vValRetSpei = '0';
                END IF;
                 
                IF vValRetSpei = '1' THEN
                    SELECT COUNT(*)
                      INTO vCteNivel1
