DROP PROCEDURE IF EXISTS "informix".sp_comision_anual_devolucion(CHAR(3), CHAR(20), CHAR(8) );
...
LET mSegCobPrincp   = 0;
	...
	--SET DEBUG FILE TO "/resplogifx/repaclaraciones/sp_comision_anual_devolucion.out";
	--TRACE ON;
    -------------------------------------------------------------------------
    --                          CONTROL DE ERRORES                         --
    -------------------------------------------------------------------------
...
BEGIN
    ...
    IF ( nvl(dFechaPreDevol, date(1)) > date(1) AND nvl(dFechaDevol, date(1)) = date(1) AND nvl(dFechaTrspDevol,date(1)) = date(1)) 
	   AND dSdo_Capital != 0  THEN 
        LET cCod_ret = '00000';
        LET cMen_ret = 'Saldo incorrecto. Es necesario realizar el retiro de la devoluci�n.';  -- Termina flujo, es necesario pasar a ventanilla
    ...
    IF (nvl(dFechaPreDevol, date(1)) > date(1) AND nvl(dFechaDevol, date(1)) > date(1) AND nvl(dFechaTrspDevol,date(1)) = date(1)
		...
		IF lower(pusuario) = 'informix' THEN  -- Si se ejecuta desde el job nocturno permita cancelar credito y deja continuar sp de cancelar.
			LET cCod_ret = '00000';
			LET cMen_ret = 'Devolucion correcta. Se cancelar� el credito';
			LET dMntoDevol = 0;
			RETURN cCod_ret, cMen_ret, dMntoDevol;
	...
	-- Obtiene el numero de dia de diferencia entre los dos a�os
    LET sDiasTotAnio = mdy('01','01',year(dFech_prox_an)) - mdy('01','01',year(dFech_prev_an));
	
    -- Obtiene los dias no devengados. Dias que no se cubrieron para cumplir el a�o. sDiasTransCob = Contiene  los dias transcurridos
	...
	--	Dias total del a�o - Dias transcurridos = dias no devengados.	// Dias no devengados sean menor a 30, no aplica devolucion.
    ...
    -- Verifica estatus de precancelacion y ya realiz� el retiro de la devolucion. Para concluir proceso de devolucion de anualidad.
    IF (nvl(dFechaPreDevol, date(1)) > date(1) AND nvl(dFechaDevol, date(1)) = date(1) AND nvl(dFechaTrspDevol,date(1)) = date(1)
        ...
        --BEGIN WORK;
            ...
            -- Elimina la marca de cr�dito bloqueado.
    ...
    IF dMntoDevolTit > 0 THEN
        ...
        IF dMntoIvaCobr > 0 THEN
            ...
            --- Realiza el deposito de la devoluci�n del IVA
