...
-- >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Actualizaciones
   -- Mod			Prod			Versión		Usr		Detalle
   -- 06/02/2013 	19/03/2013,		V0, 		SADVC,  Creación de lógica para afectaciones de Captación.
   -- 06/02/2013, 	19/03/2013,    	V1, 		SADVC,  Agregar lógica para inversiones
   -- 20/03/2013, 	21/03/2013,		V2, 		SADVC, 	Agregar validación para comisiones sin transacción flujo NP, CM
   -- 22/03/2013,	26/03/2013,		V3, 		SADVC,  Agregar validación para flujos AA, R, NP, no cargar montos no abonados
   -- 08/04/2013,	09/04/2013,		V4, 		SADVC,  Agregar validación para montos en cero y procede en 1 por empleados de ACL
   -- 16/04/2013,  	16/04/2013,		V5, 		SADVC,  Modificación envío de num_empleado que autoriza las afectaciones Entrega III, CNBV
   ...
   -- Nota. Considerar selección de transacción, cuando esten definidas las ristras contables correspondientes. (fky_tipo_movimiento = is not null --> fky_tipo_catalogo_transaccion)
...
-- >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Desactivar debug
   ...
   BEGIN WORK;
		...
		-- ***********************************************************************************************************************************************
		-->> Flujo de aclaraciones: Abonar y Analizar, No Procede = Cargo de monto previamente Abonado + Cobro de comisión.
		-->> Flujo de aclaraciones: Abonar y Analizar, No Procede = Sin afectación + Cobro de comisión.
			...
			IF (pDictamen = 'NP') THEN
				...
				IF (Es_Nacional IS NULL OR Es_Nacional = '' OR Es_Nacional = 'N') THEN --Deshabilitar cuando se utilice completamente el campo tipo_movimiento de acl_aclaracion
					...
					END IF;
				END IF;
				-------------- >> Inserta movimiento de comisión por Aclaracion NO Procedente
				...
				AND duplicado = 0
				AND d.trans_no_procede IS NOT NULL -->> Validación para comisiones sin transacción flujo NP, CM
				AND folio_csuac = pFolioSuac;
				...
				WHERE (folio_csuac = pFolioSuac AND cargo = 0 AND (exitoso = 0 OR exitoso IS NULL))
					OR (folio_csuac = pFolioSuac AND exitoso IS NULL AND fecha_afectacion IS NULL AND duplicado = 0); -->> 22/03/2013 Validación para flujos AA, R, NP, no cargar montos no abonados
		
				IF ( v_numero_transaccion IS NULL) THEN  -->> Valida si ya se ingreso la comisión de debito, para no duplicarla 24/04/2012
					If Mcosto = '0' Then
...
-->> Flujo de aclaraciones: Analizar, No Procede = Sin Afectación  --> Solo cobro de comisión.
	BEGIN WORK;
			...
			-------------- >> Inserta movimiento de comisión por Aclaracion NO Procedente
				...
				IF ( v_numero_transaccion IS NULL) THEN  -->> Valida si ya se ingreso la Comisión de Debito, para no duplicarla.
					If Mcosto = '0' Then
						INSERT INTO bdiaclaracion:acl_movimiento
						-- pky_movimiento                            calculado     cargo     cargo_ajuste	exitoso     fecha_afectacion        fecha_hora_e_global     fechahora               folio_csuac     folio_suc         identificador_adquiriente     iso_37     iso_41     monto     montoprocedente     duplicado     numero_transaccion     procede     referencia    referencia23    reversado     secuencia           fky_aclaracion     fky_padre        fky_producto     fky_solicitud_e_global     fky_tipo_evento     fky_tipo_movimiento     fky_tipo_catalogo_transaccion     ref_comercio   num_sucursal
						VALUES(bdiaclaracion:MOVIMIENTO_SEQ.nextval, 0,            1,        null,			0,          null,                   null,                   current,                pFolioSuac,     null,             null,                         null,      null,      Mcosto,   Mcosto,             0,            Ctrans_no_procede,     0,          '',           '',             0,            CSecuencia_acl_mov,  null,             Ipky_movimiento, Ifky_producto,   null,                      1,                  Ipky_tipo_movimiento,   null,                             null,          "9250", null, 0, 0);
...
-->> Proceso para Calculo de Intereses Debito
	...
	BEGIN WORK;
			...
			--> Consulta de datos de aclaración.
				...
				--> Consulta para validación de intereses.
...
-->>		 ***************************************************************************************************************************************************** <<--
		...
		-->> Validación de intereses en 0 anteriores
	...
	BEGIN WORK;
		-->> FORECH para afectación de movimientos.
			...
			--> Validación de Inversión
				...
				ELSE
					...
					SELECT numero_cuenta_inversion --> Cambiar Cuenta Inversión / Pagares por la Cuenta eje Captación.
					INTO CnumCuenta
					FROM bdiaclaracion:acl_producto a, bdiaclaracion:acl_aclaracion b
					WHERE b.fky_producto = a.pky_producto
			...
			LET CnumCuenta = CnumCuenta;
				...
				-----
					...
					IF (DCodret_a = "207") THEN
						LET cCodRet = '207'; -- Intento de cargo con crÃ?Â?Ã?Â©dito vencido "BT" y bloqueado
						ROLLBACK WORK;
						IF (wBegin = "S") THEN
							BEGIN WORK;
						END IF;
				...
				-------------- >> Actualización
					...
					ELIF (pDictamen IN ('AS')) THEN
						...
						SET cargo = 0,
							exitoso = '1',
							procede = null, --> Null para identificar si es un flujo Abono sin Autorización
							fecha_afectacion = CURRENT,
							numero_transaccion = ptranaplica,
							secuencia = CSecuencia_acl_mov
						WHERE pky_movimiento = CSecuencia
					...
					ELIF (pDictamen IN ('NP', 'CM')) THEN
						...
						ELSE
							...
							WHERE pky_movimiento = CSecuencia
						AND folio_csuac = pFolioSuac;
						END IF; -- Validar cobro de comisión
